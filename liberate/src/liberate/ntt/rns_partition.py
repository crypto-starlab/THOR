...
class RnsPartition:
    def __init__(self, num_ordinary_primes=17, num_special_primes=2, num_devices=2):
    ...
    def compute_destination_arrays(self):
        ...
        special_removed = lambda i: [a[: -self.num_special_primes] for a in self.destination_arrays_with_special[i]]

        self.destination_arrays = [special_removed(i) for i in range(self.num_ordinary_primes)]
    ...
    def partings(self, lvl):
        ...
        level_diffs = [
            len(a) - len(b)
            for a, b in zip(self.destination_arrays_with_special[0], self.destination_arrays_with_special[lvl],
            )
        ]
    ...
    def compute_partitions(self):
        ...
        self.d = [self.destination_arrays[0][dev_i] for dev_i in range(self.num_devices)]

        self.d_special = [self.destination_arrays_with_special[0][dev_i] for dev_i in range(self.num_devices)]
