...
class CkksContext:
    def __init__(
        ...
        try:
            ...
            bootstrapping_primes = generate_scale_primes(cache_folder=cache_folder, how_many=how_many)[59, self.N][:15]
        except KeyError as e:
        ...
        try:
            if num_scales is None:
                ...
                base_special_bits = sum(
                    [math.log2(p) for p in bootstrapping_primes]
                )
                available_bits -= base_special_bits
                num_scales = 0 
            ...
            self.num_scales = num_scales + 15
            self.q = bootstrapping_primes + scale_primes[:num_scales] + base_special_primes
