...
from .context.ckks_context_old import CkksContext
from .create_switcher import create_switcher_cuda as csc
from .data_struct_old import DataStruct
...
class ckks_engine:
    ...
    def __init__(self, devices: list[int] = None, verbose: bool = False,
        ...
        self.rng = Csprng(self.ntt.ctx.N, [len(di) for di in self.ntt.p.d], rng_repeats, devices=self.ntt.devices, torch_dtype=self.ctx.torch_dtype)
    ...
    def create_public_key(self, sk: DataStruct, include_special: bool = False,
                          a: list[torch.Tensor] = None) -> DataStruct:
        """
    ...
    def decrypt_double(self, ct: DataStruct, sk: DataStruct, final_round=True) -> list[torch.Tensor]:
        """
    ...
    def create_key_switching_key(self, sk_from: DataStruct, sk_to: DataStruct, level=None, a=None) -> DataStruct:
        """
    ...
    def switch_key(self, ct: DataStruct, ksk: DataStruct, is_fast=True) -> DataStruct:
    ...
    def decompose(self, a: list[torch.Tensor], level, exit_ntt=False) -> tuple:
        # ksk parts allocation.
        ksk_alloc = self.parts_alloc[level]
    ...
    def cc_mult(self, a: DataStruct, b: DataStruct, evk: DataStruct, relin=True) -> DataStruct:
    ...
    def create_hoisting_rotation_key(self, sk: DataStruct, delta: int, a: list[torch.Tensor] = None) -> DataStruct:
        ...
        rotk = self.create_key_switching_key(sk_copy, sk_rotated, a=a)
        rotk = rotk._replace(origin=types.origins["rotk"] + f"{delta}")
    ...
    # Level up.
    # -------------------------------------------------------------------------------------------
    def level_up(self, ct: DataStruct, dst_level: int) -> DataStruct:
    ...
    def conjugate(self, ct: DataStruct, conjk: DataStruct, is_fast=True):
        ...
        conj_ct = self.switch_key(conj_ct_sk, conjk, is_fast=is_fast)
        return conj_ct
    ...
    def move_to(self, text:DataStruct, direction='gpu2cpu'):
        if not isinstance(text.data[0], DataStruct):
            level = text.level
            include_special = text.include_special
    ...
    def mean(self, ct: DataStruct, gk: DataStruct, alpha=1) -> DataStruct:
        # Divide by num_slots.
    ...
    def cov(self, ct_a: DataStruct, ct_b: DataStruct, evk: DataStruct, gk: DataStruct) -> DataStruct:
    ...
    def pow(self, ct: DataStruct, power_: int, evk: DataStruct) -> DataStruct:
        current_exponent = 2
        pow_list = [ct]
        while current_exponent <= power_:
            current_ct = pow_list[-1]
            new_ct = self.cc_mult(current_ct, current_ct, evk)
            pow_list.append(new_ct)
            current_exponent *= 2

        remaining_exponent = power_ - current_exponent // 2
        new_ct = pow_list[-1]
    ...
    def square(self, ct: DataStruct, evk: DataStruct, relin=True, is_fast: bool = True) -> DataStruct:
        x = self.rescale(ct)
    ...
    def multiparty_create_public_key(self, sk: DataStruct, a=None, include_special=False) -> DataStruct:
        if sk.origin != types.origins["sk"]:
            raise errors.NotMatchType(origin=sk.origin, to=types.origins["sk"])
        if include_special and not sk.include_special:
            raise errors.SecretKeyNotIncludeSpecialPrime()
    ...
    def multiparty_create_key_switching_key(self, sk_src: DataStruct, sk_dst: DataStruct, a=None) -> DataStruct:
    ...
    def multiparty_create_rotation_key(self, sk: DataStruct, delta: int, a=None) -> DataStruct:
        sk_new_data = [s.clone() for s in sk.data]
        self.ntt.intt(sk_new_data)
        sk_new_data = [rotate(s, delta) for s in sk_new_data]
        self.ntt.ntt(sk_new_data)
